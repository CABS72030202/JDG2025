 ...
 * --------------------------------
 * This library provides an interface for establishing Bluetooth RFCOMM communication
 * between two devices, such as Raspberry Pi boards. It allows a device to operate
 * as either a server or a client, enabling bi-directional communication over Bluetooth.
 *
 * Features:
 * - Initialize Bluetooth RFCOMM server or client sockets.
 ...
 * - Linux-based systems with Bluetooth support
 * - Check /boot/firmware/config.txt to make sure that bluetooth is enabled on both RPI
 *
